    ...
    >
      ...
      <div style={{ paddingLeft: 8 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              {Icon && <Icon size={13} color={sc.color} style={{ animation: client.status === 'running' ? 'spin 1s linear infinite' : 'none' }} />}
            </div>
          ...
          <div style={{ display: 'flex', gap: 6 }} onClick={e => e.stopPropagation()}>
            ...
            <button className="btn-icon" onClick={() => onDelete(client.clientId)} title="Delete client">
        ...
        <div style={{
          ...
          background: '#1a1a24', border: '1px solid #2a2a3d', marginBottom: 12,
        }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#555570', fontSize: 11 }}>
            {client.lastRunAt
              ? <><Clock size={10} /><span style={{ fontFamily: "'Space Mono', monospace" }}>{formatDistanceToNow(new Date(client.lastRunAt), { addSuffix: true })}</span></>
              : <span>Never run</span>
            }
...
export default function ClientsPage() {
  ...
  const { clients, fetchClients, removeClient, loading, error } = useClientStore()
  const [showAdd, setShowAdd] = useState(false)

  useEffect(() => {
    fetchClients().catch(() => toast.error('Failed to load clients from database'))
  }, [])

  async function handleDelete(clientId) {
    if (!confirm('Remove this client? This will not delete any data from Azure Blob Storage.')) return
    try {
  ...
  return (
    <div className="animate-in">
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: 60, color: '#555570' }}>
          <Loader size={24} style={{ animation: 'spin 1s linear infinite', marginBottom: 12 }} />
          <p style={{ fontSize: 13 }}>Loading clients...</p>
        </div>
      )}
      ...
      {error && !loading && (
        <div style={{
          ...
          Failed to load clients: {error} — check your API key and backend URL in Settings.
        </div>
      )}
      ...
      {!loading && clients.length === 0 && !error && (
        <div style={{
          ...
          <div style={{
            width: 64, height: 64, background: 'rgba(108,99,255,0.1)',
            border: '1px solid rgba(108,99,255,0.2)', borderRadius: '50%',
      ...
      {showAdd && <AddClientModal onClose={() => { setShowAdd(false); fetchClients() }} />}
    </div>
  )
}